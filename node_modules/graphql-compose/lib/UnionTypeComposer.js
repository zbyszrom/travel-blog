  ...
  static createTemp(typeDef) {
    ...
    if ((0, _is.isString)(typeDef)) {
      ...
      if (NAME_RX.test(typeName)) {
        UTC = new this.schemaComposer.UnionTypeComposer(new _graphql.GraphQLUnionType({
          name: typeName,
          types: () => []
        }));
      } else {
      ...
      const type = new _graphql.GraphQLUnionType(_objectSpread({}, typeDef, {
        types: (0, _is.isFunction)(types) ? () => (0, _configAsThunk.resolveTypeArrayAsThunk)(this.schemaComposer, types(), typeDef.name) : () => []
      }));
      UTC = new this.schemaComposer.UnionTypeComposer(type);
      if (Array.isArray(types)) UTC.setTypes(types);
      UTC.gqType._gqcExtensions = typeDef.extensions || {};
    } else {
      throw new Error('You should provide GraphQLUnionTypeConfig or string with union name or SDL definition');
    }
    ...
    try {
      for (var _iterator2 = typeResolversMap.entries()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        try {
          ...
          throw new Error(`For union type resolver ${this.getTypeName()} you must provide GraphQLObjectType or TypeComposer, but provided ${(0, _misc.inspect)(composeType)}`);
        } // checking checkFn
