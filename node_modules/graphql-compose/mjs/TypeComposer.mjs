...
import { getComposeTypeName } from './utils/typeHelpers'; // import { deprecate } from './utils/debug';

import { graphqlVersion } from './utils/graphqlVersion';
export function isComposeOutputType(type) {
  return isType(type) || Array.isArray(type) && isComposeOutputType(type[0]) || type instanceof Resolver || type instanceof TypeComposer || type instanceof InterfaceTypeComposer || type instanceof EnumTypeComposer || type instanceof UnionTypeComposer || type instanceof ScalarTypeComposer;
} // Compose Args -----------------------------
  ...
  static createTemp(typeDef) {
    ...
    if (isString(typeDef)) {
      ...
      const type = new GraphQLObjectType(_objectSpread({}, typeDef, {
        fields: isFunction(fields) ? () => resolveOutputConfigMapAsThunk(this.schemaComposer, fields(), typeDef.name) : () => ({})
      }));
      TC = new this.schemaComposer.TypeComposer(type);
