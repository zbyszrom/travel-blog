  ...
  static createTemp(typeDef) {
    ...
    if (isString(typeDef)) {
      ...
      if (NAME_RX.test(typeName)) {
        UTC = new this.schemaComposer.UnionTypeComposer(new GraphQLUnionType({
          name: typeName,
          types: () => []
        }));
      } else {
      ...
      const type = new GraphQLUnionType(_objectSpread({}, typeDef, {
        types: isFunction(types) ? () => resolveTypeArrayAsThunk(this.schemaComposer, types(), typeDef.name) : () => []
      }));
      UTC = new this.schemaComposer.UnionTypeComposer(type);
  ...
  _isTypeResolversValid(typeResolversMap) {
    ...
    for (var _iterator2 = typeResolversMap.entries(), _isArray2 = Array.isArray(_iterator2), _i4 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
      ...
      try {
        ...
        throw new Error(`For union type resolver ${this.getTypeName()} you must provide GraphQLObjectType or TypeComposer, but provided ${inspect(composeType)}`);
      } // checking checkFn
