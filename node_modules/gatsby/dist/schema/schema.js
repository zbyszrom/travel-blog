...
const _require2 = require(`./types/node-interface`),
      addNodeInterfaceFields = _require2.addNodeInterfaceFields;

const _require3 = require(`./infer`),
      addInferredType = _require3.addInferredType,
      addInferredTypes = _require3.addInferredTypes;

const _require4 = require(`./resolvers`),
      findOne = _require4.findOne,
      findManyPaginated = _require4.findManyPaginated;

const _require5 = require(`./types/pagination`),
      getPagination = _require5.getPagination;

const _require6 = require(`./types/sort`),
      getSortInput = _require6.getSortInput;

const _require7 = require(`./types/filter`),
      getFilterInput = _require7.getFilterInput;

const _require8 = require(`./types/type-builders`),
      isGatsbyType = _require8.isGatsbyType,
      GatsbyGraphQLTypeKind = _require8.GatsbyGraphQLTypeKind;
...
function () {
  var _ref4 = (0, _asyncToGenerator2.default)(function* ({
    ...
    if (typeComposer instanceof schemaComposer.TypeComposer && typeComposer.hasInterface(`Node`)) {
...
const processAddedType = ({
  ...
  if (typeComposer instanceof schemaComposer.InterfaceTypeComposer || typeComposer instanceof schemaComposer.UnionTypeComposer) {
...
const createTypeComposerFromGatsbyType = ({
  ...
  switch (type.kind) {
    case GatsbyGraphQLTypeKind.OBJECT:
      {
        return schemaComposer.TypeComposer.createTemp(Object.assign({}, type.config, {
          interfaces: () => {
    ...
    case GatsbyGraphQLTypeKind.UNION:
      {
        return schemaComposer.UnionTypeComposer.createTemp(Object.assign({}, type.config, {
          types: () => {
            if (type.types) {
              return type.types.map(typeName => schemaComposer.getTC(typeName).getType());
            } else {
              return [];
            }
          }
...
function () {
  var _ref5 = (0, _asyncToGenerator2.default)(function* (tc) {
    if (tc instanceof schemaComposer.TypeComposer && tc.hasInterface(`Node`)) {
...
const reportParsingError = error => {
  ...
  if (source && locations && locations.length) {
    ...
    const _require9 = require(`@babel/code-frame`),
          codeFrameColumns = _require9.codeFrameColumns;
